...
def main(BATCH_SIZE, NUM_EPOCHS, TRAIN_SIZE, WEIGHT_DECAY, LEARNING_RATE):
    ...
    folder = "gretel_70"
    data_path = os.path.join(current_dir, f"../data/{folder}", "train_data.csv")
    synthetic_data_path = os.path.join(
        current_dir, f"../data/{folder}", "cleaned_synthetic_data.csv"
    )
    val_data_path = os.path.join(current_dir, f"../data/{folder}", "test_data.csv")
    ...
    df = pd.DataFrame(
        np.append(data.values, synthetic_data, axis=0), columns=data.columns
    )
    df = df.sample(frac=1).reset_index(drop=True)

    y = df[RESPONSE_VARIABLES].values
    X = df.drop(df.columns[[9, 10, 11]], axis=1).values

    y_val = val_data[RESPONSE_VARIABLES].values
    X_val = val_data.drop(df.columns[[9, 10, 11]], axis=1).values
    ...
    with torch.no_grad():
        ...
        print("=" * 60)
        print("Total: ")
        print(f"MAE: {mae:.4f} - MAPE: {mape:.4f} - MRE: {mre:.4f}")
