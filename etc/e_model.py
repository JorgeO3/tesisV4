...
def main(BATCH_SIZE, NUM_EPOCHS, TRAIN_SIZE, WEIGHT_DECAY, LEARNING_RATE):
    ...
    y_val = val_data["%E"].values.reshape(-1, 1)
    ...
    for i in range(NUM_EPOCHS):
        ...
        print(f"=========== MSE - EPOCH: {i} ==========", file=sys.stdout)
        print(f"MSE: {mse}, R2: {r2}", file=sys.stdout)
        print("========================================\n", file=sys.stdout)
    ...
    print("MSE: %.2f" % best_mse, file=sys.stdout)
    print("RMSE: %.2f" % np.sqrt(best_mse), file=sys.stdout)
    ...
    with torch.no_grad():
        ...
        for i in range(len(X_val)):
            ...
            targets_df = pd.DataFrame(targets, columns=["%E"])
            preds_df = pd.DataFrame(preds, columns=["%E"])
            ...
            print(
                f"MAE: {mae:.4f} - MAPE: {mape:.4f} - RMSE: {rmse:.4f}", file=sys.stdout)
            print("\nInputs:\n", inputs_df.to_string(
                index=False), file=sys.stdout)
