...
def make_predictions(model, features, targets, scaler_x, scaler_y):
    ...
    with torch.no_grad():
        for i in range(len(features)):
            ...
            input_feature = scaler_x.inverse_transform(input_feature.numpy().reshape(1, -1))
            prediction = scaler_y.inverse_transform(prediction.numpy().reshape(1, -1)).flatten()
            target = scaler_y.inverse_transform(target.numpy().reshape(1, -1)).flatten()
...
def main(batch_size, num_epochs, train_size, weight_decay, learning_rate):
    ...
    # power = PowerTransformer(method="yeo-johnson", standardize=False).fit(y_train)

    # y_train = power.transform(y_train)
    # y_test = power.transform(y_test)
    # y_val = power.transform(y_val)
    ...
    # _mre_list = make_predictions(model, x_val, y_val, scaler_x, scaler_y, power)
